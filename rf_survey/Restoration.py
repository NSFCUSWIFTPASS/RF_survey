...
class Restoration(object):
    def check_pulse(self):
        if os.path.exists(os.environ["HOME"] + "/lifesigns.json"):
            ...
            start_time = datetime.strptime(
                self.configs["start_time"], "%Y-%m-%d %H:%M:%S.%f"
            ) + relativedelta(seconds=self.configs["delay"])

            seconds = float(start_time.strftime("%S.%f"))
            next_interval = int(
                seconds // self.configs["interval"] * self.configs["interval"]
                + self.configs["interval"]
            )

            if next_interval >= 60:
                next_second = (
                    ((start_time.minute * 60 + start_time.second) // next_interval + 1)
                    * next_interval
                    % 60
                )
                minutes = next_interval // 60
                minute_delta = abs(
                    start_time.minute - ((start_time.minute // minutes + 1) * minutes)
                )
                self.start_time = datetime(
                    ...
                    next_second,
                    0,
                ) + relativedelta(minutes=+minute_delta)
            else:
            ...
            self.steps = (
                self.configs["end_frequency"] - self.configs["start_frequency"]
            ) / self.configs["sampling_rate"] + 1
            if self.configs["cycles"] == 0:
                end_time = datetime.today() + relativedelta(minutes=+1)
            else:
                end_time = self.start_time + relativedelta(
                    seconds=+(
                        self.steps
                        * self.configs["recordings"]
                        * self.configs["cycles"]
                        * self.configs["interval"]
                    )
                )
    ...
    def cardioversion(self):
        ...
        if self.configs["start_frequency"] != self.configs["end_frequency"]:
            ...
            recordings_left = (
                self.configs["recordings"] - n_recordings % self.configs["recordings"]
            )
            start_index = self.configs["recordings"] - recordings_left
            next_frequency = self.configs["start_frequency"] + self.configs[
                "sampling_rate"
            ] * (n_recordings // self.configs["recordings"])
        else:
            steps = 1
            n_cycles = 1
