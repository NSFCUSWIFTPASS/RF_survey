...
from rf_survey.models import ReceiverConfig, RawCapture
...
class Receiver:
    ...
    async def receive_samples(self, center_freq_hz: int) -> Optional[RawCapture]:
        """
        ...
        self.logger.debug(f"Simulating a capture of {capture_duration:.3f} seconds...")
        await asyncio.sleep(capture_duration)

        self.logger.info("MockReceiver: Capture complete. Building RawCapture object.")
